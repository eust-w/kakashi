...
9. `Verifier` detects package manager and project commands from root and nested manifests, then runs install/build/test/lint/start checks. Server readiness is not accepted from logs alone when output exposes a local URL; Kakashi probes the URL and common health endpoints, then requires an HTTP 2xx/3xx response. Kakashi combines detected Node, Python, Go, and Rust verification steps instead of stopping at the first ecosystem, and nested project checks execute from their own directories rather than from the repository root.
